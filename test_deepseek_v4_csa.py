...
from benchmark_deepseek_v4_csa import (
    make_inputs,
    metrics,
    reference_csa_attention,
    reference_csa_compress,
)
...
def test_csa_attention_exact_shape_matches_blockwise_reference() -> None:
    values = make_inputs(1, 1701)
    ...
    candidate = deepseek_v4_csa_attention(*candidate_inputs)
    ...
    gates = {
        ...
        "query gradient": (0.0033, 0.99999),
        "local KV gradient": (0.0055, 0.99998),
        "compressed KV gradient": (0.0105, 0.99994),
...
def test_csa_producer_exact_shape_matches_overlap_reference() -> None:
    values = make_inputs(1, 1801)
    output_gradient = (
        torch.randn(
            1,
            1,
...
def test_csa_non_reentrant_checkpoint_recomputation_is_deterministic() -> None:
    values = make_inputs(1, 1901)
...
def test_csa_rejects_output_gradient_copy() -> None:
    query = torch.empty(
        1,
        _QUERY_HEADS,
        _SEQUENCE_LENGTH,
    ...
    output = deepseek_v4_csa_attention(query, local_kv, compressed_kv, sink)
...
def test_csa_rejects_unsupported_batches(batch: int) -> None:
    query = torch.empty(
    ...
    with pytest.raises(ValueError, match="unsupported.*batch"):
        deepseek_v4_csa_attention(query, local_kv, compressed_kv, sink)


def test_csa_producer_accepts_strided_frozen_metadata() -> None:
    values = make_inputs(1, 1951)
    bias_storage = torch.empty(4, 2 * 2 * _HEAD_DIM, device="cuda", dtype=torch.float32)
...
def test_csa_producer_rejects_trainable_frozen_state() -> None:
    values = make_inputs(1, 2001)
    with pytest.raises(ValueError, match="must be frozen"):
        deepseek_v4_csa_compress(
            values["compressor_kv"],
            values["compressor_gate"],
...
def test_csa_module_dispatch_rejects_selective_topk() -> None:
    module = SimpleNamespace(
        ...
        compressor=SimpleNamespace(
            compress_rate=4,
            indexer=SimpleNamespace(index_topk=511),
        ),
    )
