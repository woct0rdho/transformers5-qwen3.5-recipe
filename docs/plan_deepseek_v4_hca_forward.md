...
- source rows are `128*c <= t < 128*(c+1)`.
- compressor gate plus frozen position bias `[128,512]` is reduced with an FP32 featurewise softmax over 128 rows.
- the FP32 weighted KV pool receives frozen weighted RMSNorm.
- the normalized result is quantized to BF16 at the Transformers norm boundary.
- trailing interleaved RoPE pairs are rotated at absolute position `128*c`.
...
- local keys `max(0,t-127) <= k <= t`.
- compressed prefix `0 <= c < floor((t+1)/128)`.
- one FP32 per-head sink logit.
...
Final producer-plus-attention measurements from `~/tmp/test_no_unsloth/deepseek_v4_hca_phase_final.json` are:
...
`~/tmp/test_no_unsloth/deepseek_v4_training_hca_report.json` passes with:
...
- one grouped owner computes local and compressed QK, writes the existing separate raw-score states, and carries the shared local/compressed/sink max and denominator to FP32 LSE without a D512 output accumulator.
- D128/D256 feature-sliced owners reconstruct normalized probabilities from raw scores/LSE, traverse both KV regions, and write disjoint BF16 output slices.
- no probability tensor, second score state, output partial workspace, repeated KV head, or QK recomputation is permitted.
...
Exact B16 output/Q/local-KV/compressed-KV/sink RMSE after both accepted phases is `0.002951/0.003205/0.005225/0.011517/0.001214`. Production B16 checkpoint replay is bitwise equal for output and all four attention gradients. Evidence is in `~/tmp/test_no_unsloth/deepseek_v4_hca_phase_screen.json`, `~/tmp/test_no_unsloth/deepseek_v4_hca_forward_phase_b16.json`, `~/tmp/test_no_unsloth/deepseek_v4_hca_phase_final.json`, `~/tmp/test_no_unsloth/deepseek_v4_hca_phase_checkpoint_b16.json`, and `~/tmp/test_no_unsloth/deepseek_v4_hca_phase_metadata.json`.
