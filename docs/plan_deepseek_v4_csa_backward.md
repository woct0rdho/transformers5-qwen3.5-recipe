...
1. local key-owned `dV` reconstructs probabilities from local scores and FP32 LSE;
2. group-2 compressed key owners reconstruct probabilities into a bounded reusable FP32 partial workspace, then one deterministic reducer writes compressed `dV`;
3. at B1/B4, one grouped query owner computes `Delta = sum_d O*dO`, reconstructs both probability regions, accumulates one `dQ`, computes deterministic sink partials, and overwrites both score regions with FP16 `dS`;
4. at B16, that owner forms Delta, sink partials, and both dS regions without a D512 dQ accumulator; a second D256 feature-sliced owner consumes stored dS and writes disjoint final BF16 dQ slices;
5. local key-owned one-dot `dK` consumes local `dS` and adds into local `dV`;
6. group-2 compressed key owners reuse the FP32 workspace for dK, and the deterministic reducer adds compressed dK into dV;
7. one deterministic FP32 sink reduction combines query-tile partials.
...
`/tmp/deepseek_v4_training_csa_cleanup_report.json` passes:
