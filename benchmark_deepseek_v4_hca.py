#!/usr/bin/env python3
"""Correctness, timing, and allocation harness for fixed-shape DeepSeek V4 HCA."""
...
def reference_hca_attention(
    ...
    for query_start in range(0, _SEQUENCE_LENGTH, block_m):
        ...
        compressed_end = query_end // _COMPRESS_RATE
        ...
        compressed_visible = (
            compressed_positions[None, :] < (rows[:, None] + 1) // _COMPRESS_RATE
        )
...
def reference_hca_compress(
    ...
    batch = kv.shape[0]
    kv_windows = kv.view(batch, _COMPRESSED_LENGTH, _COMPRESS_RATE, _HEAD_DIM).float()
    gate_windows = (
        gate.view(batch, _COMPRESSED_LENGTH, _COMPRESS_RATE, _HEAD_DIM) + position_bias
    ).float()
    probabilities = gate_windows.softmax(dim=2)
    pooled = (kv_windows * probabilities).sum(dim=2)
    normalized = pooled * torch.rsqrt(
        pooled.square().mean(dim=-1, keepdim=True) + rms_eps
    )
    normalized = (normalized * weight.float()).to(torch.bfloat16)

    positions = torch.arange(_COMPRESSED_LENGTH, device=kv.device) * _COMPRESS_RATE
    cos_values = cos[0, positions].float()
    sin_values = sin[0, positions].float()
    rope = normalized[..., -64:].float().reshape(batch, _COMPRESSED_LENGTH, 32, 2)
...
def run_correctness(batch: int, seed: int) -> dict[str, object]:
    ...
    candidate_output = deepseek_v4_hca_attention(*candidate_inputs)
...
def run_benchmark(
    ...
    def forward_only() -> None:
        with torch.no_grad():
            compressed = deepseek_v4_hca_compress(
                compressor_kv,
                compressor_gate,
                values["position_bias"],
                values["weight"],
            ...
            deepseek_v4_hca_attention(query, local_kv, compressed, sink)
    ...
    def complete() -> None:
        ...
        output = deepseek_v4_hca_attention(query, local_kv, compressed, sink)
        torch.autograd.grad(
            output,
