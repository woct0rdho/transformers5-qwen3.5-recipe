...
_PRODUCER_NUM_WARPS = 8
_PRODUCER_WAVES_PER_EU = 2
_PACK_COMPRESSED_SCORES = {1: False, 4: False, 16: True}
_DENSE_COMPRESSED_SCORE_COUNT = _SEQUENCE_LENGTH * _COMPRESSED_LENGTH
_PACKED_COMPRESSED_SCORE_COUNT = (
    2 * (_COMPRESSED_LENGTH - 1) * _COMPRESSED_LENGTH + _COMPRESSED_LENGTH
)


def _compressed_score_count(pack_scores: bool) -> int:
    if pack_scores:
        return _PACKED_COMPRESSED_SCORE_COUNT
    return _DENSE_COMPRESSED_SCORE_COUNT


@triton.jit
def _compressed_score_row(query, pack_scores):
    if not pack_scores:
        return query * 512
    group = query // 4
    offset = query - group * 4
    preceding = tl.maximum(group - 1, 0)
    score_align: tl.constexpr = 1
    full_groups = preceding // score_align
    remainder = preceding - full_groups * score_align
    prefix = score_align * full_groups * (full_groups + 1) // 2 + remainder * (
        full_groups + 1
    )
    row_groups = (group + score_align - 1) // score_align
    return score_align * (4 * prefix + (offset + 1) * row_groups)
...
def _csa_forward_kernel(
    ...
    compressed_end = (start_m + BLOCK_M) // 4
    for start_n in tl.range(0, compressed_end, COMPRESSED_N, loop_unroll_factor=1):
        ...
        scores = tl.dot(query, tl.trans(kv)) * (SM_SCALE * log2e)
        visible = offs_n[None, :] < (offs_m[:, None] + 1) // 4
        score_row = _compressed_score_row(offs_m, PACK_SCORES)
        score_offsets = (
            batch * stride_cscb
            + heads[:, None] * stride_csch
            + score_row[:, None] * stride_cscm
            + offs_n[None, :] * stride_cscm
        )
...
@triton.jit
def _csa_output_from_scores_kernel(
    local_kv_ptr,
    compressed_kv_ptr,
    lse_ptr,
    output_ptr,
...
def _csa_dq_from_ds_kernel(
    ...
    for start_n in tl.range(first_key, key_end, LOCAL_N, loop_unroll_factor=1):
        ...
        grad_scores = tl.load(
            local_score_ptr + score_offsets, mask=visible, other=0.0
        ).to(tl.float32)
        grad_query = tl.dot(grad_scores.to(tl.bfloat16), kv, acc=grad_query)
...
def _local_dk_kernel(
    ...
    for head in tl.range(0, QUERY_HEADS, 1, loop_unroll_factor=HEAD_UNROLL):
        for start_m in tl.range(first_query, query_end, BLOCK_M, loop_unroll_factor=1):
            ...
            query = tl.load(query_ptr + q_offsets)
            visible = (offs_n[None, :] <= offs_m[:, None]) & (
                offs_n[None, :] >= offs_m[:, None] - WINDOW_LEFT
            )
            band = offs_n[None, :] - offs_m[:, None] + WINDOW_LEFT
...
@triton.jit
def _compressed_dv_partial_kernel(
    grad_output_ptr,
    lse_ptr,
    partial_ptr,
    score_ptr,
    ...
    log2e: tl.constexpr = 1.4426950408889634
    first_query = 4 * start_n + 3
    first_query = (first_query // BLOCK_M) * BLOCK_M
    for head_offset in tl.range(0, HEAD_GROUP, 1, loop_unroll_factor=HEAD_UNROLL):
...
@triton.jit
def _compressed_dk_partial_kernel(
    query_ptr,
    partial_ptr,
    grad_score_ptr,
    PACK_SCORES: tl.constexpr,
...
@triton.jit
def _compressed_partial_reduce_kernel(
    partial_ptr,
    grad_kv_ptr,
    stride_pb,
    stride_pg,
...
def _producer_forward_kernel(
    ...
    for slot in tl.static_range(0, 8):
        if slot < 4:
            source = (entry - 1) * 4 + slot
            half = 0
            valid = entry > 0
        else:
            source = entry * 4 + slot - 4
            half = HEAD_DIM
            valid = True
        kv = tl.load(
            kv_ptr + batch * stride_kb + source * stride_ks + (half + d) * stride_kd,
            mask=valid,
            other=0.0,
        ).to(tl.float32)
        gate = tl.load(
            gate_ptr + batch * stride_gb + source * stride_gs + (half + d) * stride_gd,
            mask=valid,
            other=float("-inf"),
        ).to(tl.float32)
        bias = tl.load(bias_ptr + (slot % 4) * stride_br + (half + d) * stride_bd).to(
            tl.float32
        )
        score = gate + bias
        next_max = tl.maximum(maximum, score)
        old_scale = tl.where(maximum == float("-inf"), 0.0, tl.exp(maximum - next_max))
        probability = tl.where(valid, tl.exp(score - next_max), 0.0)
    ...
    out_base = output_ptr + batch * stride_ob + entry * stride_oc
    tl.store(out_base + d * stride_od, output)
...
def _producer_backward_kernel(
    ...
    inverse = tl.where(
        (d & 1) == 0, grad * cos + partner_grad * sin, grad * cos - partner_grad * sin
    )
    ...
    for slot in tl.static_range(0, 8):
        ...
        probability = tl.where(valid, tl.exp(gate + bias - maximum) / denominator, 0.0)
...
def _validate_attention_inputs(
    ...
    if query.ndim != 4 or local_kv.ndim != 4 or compressed_kv.ndim != 4:
        raise ValueError("CSA query and KV tensors must be rank-4 BSHD")
    batch = query.shape[0]
    if batch not in _SUPPORTED_BATCHES:
        raise ValueError(f"unsupported DeepSeek V4 CSA batch {batch}")
    expected = {
    ...
    if any(tensor.device != query.device for tensor in (local_kv, compressed_kv, sink)):
        raise ValueError("all CSA tensors must share one device")
...
def _attention_forward(
    ...
    batch = query.shape[0]
    pack_scores = _PACK_COMPRESSED_SCORES[batch]
    ...
    compressed_scores = torch.empty(
        (batch, _QUERY_HEADS, _compressed_score_count(pack_scores)),
        device=query.device,
        dtype=torch.float16,
    )
    if batch == 16:
        config = _FORWARD_PHASE_CONFIG
        _csa_score_lse_kernel[
            (
        ...
        d_slices = _HEAD_DIM // config["block_d"]
        _csa_output_from_scores_kernel[
            (
                triton.cdiv(_SEQUENCE_LENGTH, config["block_m"]),
                triton.cdiv(_QUERY_HEADS, config["head_group"]) * d_slices,
                batch,
            )
        ](
            local_kv,
...
def _launch_compressed_partial(
    kernel,
    query,
    grad_output,
    lse,
...
def _reduce_compressed_partial(partial, grad_compressed, *, add_key: bool) -> None:
    _compressed_partial_reduce_kernel[(_COMPRESSED_LENGTH, partial.shape[0])](
        partial,
        grad_compressed,
        *partial.stride(),
...
def _attention_backward(
    ...
    compressed_config = _COMPRESSED_PARTIAL_CONFIGS[batch]
    query_tiles = triton.cdiv(_SEQUENCE_LENGTH, dq_config["block_m"])
    grad_query = torch.empty_like(query)
    grad_local = torch.empty_like(local_kv)
    grad_compressed = torch.empty_like(compressed_kv)
    compressed_head_group = compressed_config["dv"]["head_group"]
    if compressed_head_group != compressed_config["dk"]["head_group"]:
        raise RuntimeError("CSA compressed dV/dK head groups must match")
    compressed_partial = torch.empty(
        (
            batch,
            _QUERY_HEADS // compressed_head_group,
            _COMPRESSED_LENGTH,
            _HEAD_DIM,
        ),
        device=query.device,
    ...
    _launch_compressed_partial(
        _compressed_dv_partial_kernel,
        query,
        grad_output,
        lse,
        compressed_partial,
        compressed_scores,
        compressed_config["dv"],
        is_key=False,
    )
    _reduce_compressed_partial(compressed_partial, grad_compressed, add_key=False)

    head_group = dq_config["head_group"]
    _csa_dq_kernel[(query_tiles, triton.cdiv(_QUERY_HEADS, head_group), batch)](
        local_kv,
        compressed_kv,
        output,
        grad_output,
    ...
    if batch == 16:
        phase_config = _DQ_PHASE_CONFIG
        d_slices = _HEAD_DIM // phase_config["block_d"]
        _csa_dq_from_ds_kernel[
            (
                triton.cdiv(_SEQUENCE_LENGTH, phase_config["block_m"]),
                triton.cdiv(_QUERY_HEADS, phase_config["head_group"]) * d_slices,
                batch,
            )
        ](
            local_kv,
            ...
            *compressed_scores.stride(),
            SM_SCALE=_SOFTMAX_SCALE,
            BLOCK_M=phase_config["block_m"],
            LOCAL_N=dq_config["local_n"],
            COMPRESSED_N=dq_config["compressed_n"],
            BLOCK_D=phase_config["block_d"],
            HEAD_DIM=_HEAD_DIM,
            SEQUENCE_LENGTH=_SEQUENCE_LENGTH,
            WINDOW_LEFT=_WINDOW_LEFT,
            HEAD_GROUP=phase_config["head_group"],
            num_warps=phase_config["num_warps"],
            num_stages=1,
            waves_per_eu=phase_config["waves_per_eu"],
        )
    ...
    _launch_compressed_partial(
        _compressed_dk_partial_kernel,
        query,
        grad_output,
        lse,
        compressed_partial,
        compressed_scores,
        compressed_config["dk"],
        is_key=True,
    )
...
class _CSAAttentionFunction(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx: Any, grad_output):
        ...
        if grad_output.dtype != torch.bfloat16 or grad_output.stride(-1) != 1:
            raise ValueError(
                "CSA output gradient must be BF16 with a contiguous last dimension"
            )
...
def deepseek_v4_csa_attention(
    ...
    if query.ndim != 4 or local_kv.ndim != 4 or compressed_kv.ndim != 4:
        raise ValueError("CSA query and KV tensors must be rank-4 BHSD")
    ...
    return _CSAAttentionFunction.apply(query_bshd, local_bshd, compressed_bshd, sink)


def _validate_producer_inputs(kv, gate, bias, weight, cos, sin) -> None:
    if kv.ndim != 3 or gate.ndim != 3:
        raise ValueError("CSA producer projections must be rank-3")
    batch = kv.shape[0]
    if batch not in _SUPPORTED_BATCHES:
        raise ValueError(f"unsupported DeepSeek V4 CSA producer batch {batch}")
    expected = (batch, _SEQUENCE_LENGTH, 2 * _HEAD_DIM)
    if tuple(kv.shape) != expected or tuple(gate.shape) != expected:
        raise ValueError(f"CSA producer projections must have shape {expected}")
    if any(tensor.dtype != torch.bfloat16 for tensor in (kv, gate, cos, sin)):
        raise TypeError("CSA producer activations and RoPE tables must use BF16")
    if any(
        tensor.dtype not in (torch.bfloat16, torch.float32) for tensor in (bias, weight)
    ):
        raise TypeError("CSA producer bias and RMS weight must use BF16 or FP32")
    if any(tensor.requires_grad for tensor in (bias, weight, cos, sin)):
        raise ValueError(
            "CSA producer bias, RMS weight, and RoPE tables must be frozen"
        )
    if tuple(bias.shape) != (_COMPRESS_RATE, 2 * _HEAD_DIM):
        raise ValueError("CSA position bias must have shape [4,1024]")
    if tuple(weight.shape) != (_HEAD_DIM,):
        raise ValueError("CSA RMS weight must have shape [512]")
    if (
        cos.ndim != 3
        ...
        raise ValueError("CSA cos/sin must have matching [1|B,2048,32] layouts")
    tensors = (kv, gate, bias, weight, cos, sin)
    if any(tensor.device != kv.device for tensor in tensors):
        raise ValueError("all CSA producer tensors must share one device")
    if kv.stride(-1) != 1 or gate.stride(-1) != 1 or weight.stride(0) != 1:
        raise ValueError(
            "CSA producer activations and RMS weight require contiguous last dimensions"
        )
...
def _producer_forward(kv, gate, bias, weight, cos, sin, rms_eps: float):
    ...
    _producer_forward_kernel[(_COMPRESSED_LENGTH, batch)](
        ...
        *output.stride(),
        ...
        HEAD_DIM=_HEAD_DIM,
        num_warps=_PRODUCER_NUM_WARPS,
        num_stages=1,
        waves_per_eu=_PRODUCER_WAVES_PER_EU,
    )
    return output
...
class _CSAProducerFunction(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx: Any, grad_output):
        ...
        if grad_output.dtype != torch.bfloat16 or grad_output.stride(-1) != 1:
            raise ValueError(
                "CSA producer gradient must be BF16 with a contiguous last dimension"
            )
        _require_int32_offsets("grad_output", grad_output)
...
def deepseek_v4_csa_compress(
    ...
    """Produce rate-4 overlap-compressed shared KV as contiguous BHSD."""
    _validate_producer_inputs(kv, gate, bias, weight, cos, sin)
    return _CSAProducerFunction.apply(
